...
function App() {
  ...
  const transformBibleReferences = (text) => {
    ...
    const bibleRefRegex =
      /(\d?\s*[A-ZÀ-ÿ][a-zà-ÿ]*(?:\s+\d*\s*[A-ZÀ-ÿ][a-zà-ÿ]*)*)\s+(\d+)(?::(\d+)(?:[-–](\d+))?)?(?!\d)/g;
  ...
  const parseRubriquesContent = (fullContent) => {
    ...
    for (let i = 1; i < sections.length; i += 2) {
      ...
      console.log(
        `[PARSING DEBUG] Rubrique ${rubriqueNumber}:`,
        rubriqueContent.length,
        'caractères',
      );
      ...
      if (rubriqueContent) {
        ...
        if (rubriqueContent.length > 0) {
          ...
          console.log(
            `[PARSING SUCCESS] Rubrique ${rubriqueNumber} parsée:`,
            rubriqueContent.substring(0, 100) + '...',
          );
        }
      }
    }
  ...
  const generateRubriqueContent = (
    ...
    return (
      contenuParRubrique[rubriqueNum] ||
...
Ainsi, notre prière d'ouverture devant ${passage} n'est pas une simple formalité liturgique, mais l'expression de notre théologie de la révélation : Dieu se révèle, l'homme reçoit par grâce, et l'Esprit rend témoignage à la vérité divine dans le cœur du croyant régénéré.`
  ...
  const generateTheologicalFoundationsContent = (passage, book, chapter, targetLength) => {
    ...
    if (targetLength >= 500) {
      return (
        baseContent +
        ` La création ex nihilo manifeste la toute-puissance divine. L'anthropologie biblique trouve ici ses racines : l'homme, créé à l'image de Dieu (imago Dei), reçoit une dignité unique dans la création.`
      );
    }
    ...
    if (targetLength >= 1000) {
      return (
        baseContent +
        ` La création ex nihilo manifeste la toute-puissance divine. Contrairement aux cosmogonies babyloniennes qui décrivent des théomachies primordiales, l'Écriture présente un Dieu souverain créant par sa seule parole.
...
Le sabbat révèle la pédagogie divine. En se reposant le septième jour, Dieu établit un modèle anthropologique : l'alternance entre activité créatrice et contemplation adoratrice. Cette institution sabbatique préfigure le repos eschatologique promis au peuple de Dieu et trouve son accomplissement sotériologique en Christ, notre repos sabbatique véritable selon l'épître aux Hébreux.`
    ...
    if (targetLength >= 2000) {
      return (
        baseContent +
        ` La création ex nihilo (à partir du néant) manifeste la toute-puissance divine et constitue un dogme fondamental distinguant radicalement la foi biblique de toute philosophie naturaliste ou panthéiste. Contrairement aux cosmogonies babyloniennes (Enuma Elish) qui décrivent des théomachies primordiales, l'Écriture présente un Dieu souverain créant par sa seule parole efficace.
...
- Un témoignage à partager de l'impact de ce texte`;
  ...
  const openYouVersion = () => {
    ...
    if (!code) return alert('Livre non reconnu pour YouVersion');
  ...
  const navigateToRubrique = (rubriqueNumber, content = '') => {
    ...
    console.log(
      `[NAVIGATE RUBRIQUE DEBUG] Content reçu:`,
      content ? content.length : 0,
      'caractères',
    );
    console.log(
      `[NAVIGATE RUBRIQUE DEBUG] Content preview:`,
  ...
  const continueVerses = async () => {
    try {
      ...
      console.log(
        `[CONTINUE VERSETS] Génération versets ${startVerse} à ${endVerse} pour ${passage}`,
      );
  ...
  const handleRubriqueSelect = async (id) => {
    ...
    if (rubriquesStatus[id] === 'completed' && generatedRubriques[contentKey]) {
      console.log(
        `[AFFICHAGE RUBRIQUE ${id}] Contenu sauvegardé trouvé - longueur: ${generatedRubriques[contentKey].length}`,
      );

      // Si rubrique 0 et qu'il y a du contenu verset par verset, naviguer vers la page verset dédiée
      if (
        id === 0 &&
        (generatedRubriques[contentKey].includes('VERSET') ||
          generatedRubriques[contentKey].includes('**TEXTE BIBLIQUE'))
      ) {
        const bookInfo = `${selectedBook || 'Genèse'} ${selectedChapter || '1'}${selectedVerse !== '--' ? ':' + selectedVerse : ''}`;
  ...
  const generateRubriqueOnDemand = async (rubriqueNum) => {
    ...
    try {
      ...
      const rubriqueContent = generateRubriqueContent(
        rubriqueNum,
        rubriqueTitle,
        passage,
        selectedBook,
  ...
  const changePalette = () => {
    ...
    if (btnReset) {
      btnReset.style.background = theme.secondary;
    ...
    if (btnPalette) {
      btnPalette.style.background = theme.primary;
      btnPalette.style.color = 'white';
      btnPalette.style.border = 'none';
    }
    ...
    if (btnLastStudy) {
      ...
      btnLastStudy.style.color = 'white';
      btnLastStudy.style.border = 'none';
    }

    const btnGemini = document.querySelector('.btn-gemini');
    if (btnGemini) {
      btnGemini.style.background = `linear-gradient(90deg, ${theme.secondary}, ${theme.accent})`;
      btnGemini.style.color = 'white';
      btnGemini.style.border = 'none';
    }

    const btnVersets = document.querySelector('.btn-versets-prog');
    if (btnVersets) {
      btnVersets.style.background = theme.accent;
      btnVersets.style.color = 'white';
      btnVersets.style.border = 'none';
    }

    const btnGenerate = document.querySelector('.btn-generate');
    if (btnGenerate) {
      btnGenerate.style.background = `linear-gradient(45deg, ${theme.primary}, ${theme.secondary})`;
    ...
    if (btnValidate) {
      btnValidate.style.background = theme.primary;
  ...
  const generateWithGemini = async () => {
    try {
      ...
      if (activeRubrique >= 1 && activeRubrique <= 28) {
        ...
        console.log(
          `[ENRICHISSEMENT GEMINI GRATUIT] Rubrique ${activeRubrique} - Longueur enrichie: ${enrichedLength} caractères`,
        );
        ...
        const data = await response.json();
        console.log(
          '[GEMINI GRATUIT] Enrichissement reçu:',
          data.content ? data.content.length : 0,
          'caractères',
        );
        ...
        setContent(
          "⚠️ Pour la rubrique 0, utilisez le bouton 'VERSETS PROG' qui utilise déjà votre Gemini gratuit.",
        );
      } else {
  ...
  const generateTheologicalEnrichment = (
    rubriqueNum,
    rubriqueTitle,
    passage,
    book,
    ...
    const baseContent = generateRubriqueContent(
      rubriqueNum,
      rubriqueTitle,
      passage,
      book,
    ...
    const contextualEnrichment = getCombinedContextualEnrichment(
      book,
      chapter,
      rubriqueNum,
      passage,
...
*Enrichissement généré par analyse contextuelle approfondie*`;
    ...
    return (
      baseContent + enrichmentSuffix.substring(0, Math.max(0, targetLength - baseContent.length))
    );
  };
  ...
  const getBookSpecificEnrichment = (book, chapter) => {
    ...
    return (
      bookEnrichments[book] ||
      `**Perspective biblique :** Ce passage de ${book} ${chapter} révèle des dimensions théologiques que les commentateurs anciens et modernes ont explorées avec profit.`
    );
  };
  ...
  const getRubriqueSpecificEnrichment = (rubriqueNum, rubriqueTitle) => {
    ...
    return (
      rubriqueEnrichments[rubriqueNum] ||
      `**Enrichissement thématique :** Cette rubrique "${rubriqueTitle}" ouvre des perspectives théologiques que la tradition exégétique a particulièrement développées.`
    );
  };
  ...
  const generate28Points = async () => {
    try {
      ...
      console.log(
        '[GÉNÉRATION OPTIMISÉE] Génération immédiate de la Rubrique 1, puis les autres en arrière-plan',
      );

      // 🎯 PHASE 1: Générer et naviguer vers Rubrique 1 immédiatement
      console.log(
        "[PHASE 1] Génération et navigation immédiate vers Rubrique 1 - Prière d'ouverture",
      );
      ...
      try {
        ...
        if (response.ok) {
          ...
          console.log(
            `[NAVIGATION IMMÉDIATE] Rubrique 1 générée: ${rubrique1Content.length} caractères`,
          );
  ...
  const generateSingleRubrique = async (rubriqueNum, rubriqueTitle, passage) => {
    try {
      console.log(
        `[GÉNÉRATION RUBRIQUE ${rubriqueNum}] DEMANDÉE: "${rubriqueTitle}" pour ${passage}`,
      );
      console.log(
        `[DEBUG PARAMS] rubriqueNum=${rubriqueNum}, rubriqueTitle="${rubriqueTitle}", passage="${passage}"`,
      );
      ...
      try {
        ...
        const data = await response.json();
        console.log(
          `[API SUCCESS RUBRIQUE ${rubriqueNum}]`,
          data.content ? data.content.length : 0,
          'caractères',
        );
        ...
        console.log(
          `[PARSING] Rubriques extraites:`,
          Object.keys(rubriques).length,
          'rubriques trouvées',
        );
        console.log(`[PARSING] Clés disponibles:`, Object.keys(rubriques));
        rubriqueContent = rubriques[rubriqueNum];
        console.log(
          `[PARSING] Contenu rubrique ${rubriqueNum}:`,
          rubriqueContent ? rubriqueContent.slice(0, 100) + '...' : 'VIDE',
        );
        ...
        Object.keys(rubriques).forEach((key) => {
          console.log(
            `  Rubrique ${key}: ${rubriques[key] ? rubriques[key].slice(0, 50) + '...' : 'VIDE'}`,
          );
        });
        ...
        if (!rubriqueContent && data.content) {
          console.log(
            `[FALLBACK PARSING] Utilisation du contenu complet pour rubrique ${rubriqueNum}`,
          );
        ...
        rubriqueContent = generateRubriqueContent(
          rubriqueNum,
          rubriqueTitle,
          passage,
          selectedBook,
      ...
      console.log(
        `[RUBRIQUE ${rubriqueNum} TERMINÉE] ${rubriqueContent.length} caractères générés`,
      );

      // Debug: Vérifier le contenu avant de le retourner
      console.log(
        `[GENERATE SINGLE DEBUG] Rubrique ${rubriqueNum} contenu final:`,
        formattedContent ? formattedContent.length : 0,
        'caractères',
      );
      console.log(
        `[GENERATE SINGLE DEBUG] Contenu preview:`,
        formattedContent ? formattedContent.slice(0, 200) + '...' : 'VIDE',
      );
  ...
  const formatContent = (text, context = 'default') => {
    ...
    let formattedText = text
      ...
      .replace(
        /^\*\*TEXTE BIBLIQUE\s*:\*\*$/gim,
        "<h4 class='texte-biblique-label'>📜 TEXTE BIBLIQUE :</h4>",
      )
      .replace(
        /^TEXTE BIBLIQUE\s*:?$/gim,
        "<h4 class='texte-biblique-label'>📜 TEXTE BIBLIQUE :</h4>",
      ...
      .replace(
        /^\*\*EXPLICATION THÉOLOGIQUE\s*:\*\*$/gim,
        "<h4 class='explication-label'>🎓 EXPLICATION THÉOLOGIQUE :</h4>",
      )
      .replace(
        /^EXPLICATION THÉOLOGIQUE\s*:?$/gim,
        "<h4 class='explication-label'>🎓 EXPLICATION THÉOLOGIQUE :</h4>",
