...
const CharacterHistoryPage = ({ character, onGoBack }) => {
  ...
  const generateCharacterHistory = useCallback(async () => {
    setIsLoading(true);
    
    try {
      // Créer un AbortController pour gérer le timeout manuellement (60s pour Gemini API)
...
*Service de génération automatique - Erreur temporaire*`;
  ...
  }, [character, generateCharacterHistory]);
