...
Barak enseigne que la vraie force réside dans la dépendance envers Dieu et ses instruments oints. Sa collaboration avec Déborah illustre l'humilité et la sagesse de s'entourer de conseil spirituel. Sa foi, bien que nécessitant l'encouragement, produit des fruits durables pour la gloire de Dieu et la liberté de son peuple.`,
      ...
      const history =
        charactersDatabase[character] ||
        `# 📖 ${character.toUpperCase()} - Histoire Biblique Détaillée
    ...
    try {
      ...
      const enrichedResults = [
        ...
        // Ajouter des versets enrichis par Gemini
        {
          ...
          chapter: 8,
          verse: 17,
          text: `Les versets enrichis par Gemini pour "${searchTerm}" incluent des références croisées et des contextes approfondis...`,
        },
        {
          book: 'Psaume',
          chapter: 119,
          verse: 105,
          text: `Analyse théologique approfondie du terme "${searchTerm}" selon les commentaires bibliques et la tradition...`,
        },
      ];
...
*Enrichissement généré par Gemini AI pour une compréhension plus profonde des Écritures.*
  ...
  const generateConcordanceResults = (term) => {
    const mockVerses = {
      amour: [
        ...
        },
        {
          ...
          chapter: 3,
          verse: 19,
          text: "et connaître l'amour de Christ, qui surpasse toute connaissance, en sorte que vous soyez remplis jusqu'à toute la plénitude de Dieu.",
        },
      ],
      paix: [
        {
      ...
      joie: [
        {
          ...
          chapter: 8,
          verse: 10,
          text: "Il leur dit: Allez, mangez des viandes grasses et buvez des liqueurs douces, et envoyez des portions à ceux qui n'ont rien de préparé, car ce jour est consacré à notre Seigneur; ne vous affligez pas, car la joie de l'Éternel sera votre force.",
        },
        {
          book: 'Psaume',
          chapter: 16,
        ...
        },
        {
          ...
          chapter: 1,
          verse: 8,
          text: "vous l'aimez sans l'avoir vu, vous croyez en lui sans le voir encore, vous réjouissant d'une joie ineffable et glorieuse,",
        },
      ],
      espoir: [
        {
        ...
        },
        {
          ...
          chapter: 1,
          verse: 3,
          text: "Béni soit Dieu, le Père de notre Seigneur Jésus-Christ, qui, selon sa grande miséricorde, nous a régénérés, pour une espérance vivante, par la résurrection de Jésus-Christ d'entre les morts,",
        },
        {
          book: 'Hébreux',
          chapter: 6,
      ...
      sagesse: [
        ...
        },
        {
          ...
          chapter: 3,
          verse: 13,
          text: "Heureux l'homme qui a trouvé la sagesse, Et l'homme qui possède l'intelligence!",
        },
        {
          book: 'Ecclésiaste',
          chapter: 7,
          verse: 12,
          text: "Car à l'ombre de la sagesse on est abrité comme à l'ombre de l'argent; mais un avantage de la science, c'est que la sagesse fait vivre ceux qui la possèdent.",
        },
      ],
      pardon: [
        {
    ...
    const uniqueResults = matchingEntries.filter(
      (verse, index, arr) =>
        arr.findIndex(
  ...
  const highlightSearchTerm = (text, term) => {
    ...
    return text.replace(
      regex,
      '<mark style="background: #fef3c7; color: #92400e; padding: 2px 4px; border-radius: 4px;">$1</mark>',
    );
  };

  // Rendu conditionnel : si un thème est sélectionné, afficher ThemeVersesPage
  if (selectedTheme) {
    return <ThemeVersesPage theme={selectedTheme} onGoBack={handleBackFromTheme} />;
  }
            ...
            >
              ...
              {!selectedCharacter ? (
                <>
                  ...
                  <div
                    className="biblical-characters-grid"
                    style={{
                      display: 'grid',
                      ...
                      <div
                        dangerouslySetInnerHTML={{
                          __html: characterHistory
                            ...
                            .replace(
                              /##\s(.+)/g,
                              '<h2 style="color: #1e293b; margin: 24px 0 20px 0; font-size: 20px; font-weight: 700; border-bottom: 2px solid rgba(139, 92, 246, 0.3); padding-bottom: 8px; line-height: 1.3; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">$1</h2>',
                            )
                            // Supprimer les ### et numérotation pour un style narratif
                            .replace(
                              /###\s?\d*\.?\s*(.+)/g,
                              '<p style="color: #1e293b; margin: 16px 0 12px 0; font-size: 16px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">$1</p>',
                            )
                            // Gras pour les passages importants
                            .replace(
                              /\*\*(.+?)\*\*/g,
                              '<strong style="color: #7c3aed; font-weight: 700;">$1</strong>',
                            )
                            // Italique pour les mots étrangers et citations
                            .replace(
                              /\*(.+?)\*/g,
                              '<em style="color: #64748b; font-style: italic;">$1</em>',
                            )
                            // Versets cliquables
                            .replace(
                              /(Exode|Lévitique|Nombres|Genèse|Deutéronome|Psaumes|Hébreux|Matthieu|Marc|Luc|Jean|Actes|Romains|1 Corinthiens|2 Corinthiens|Galates|Éphésiens|Philippiens|Colossiens)\s+(\d+):(\d+(?:-\d+)?)/g,
                            ...
                            .replace(
                              /([.!?])\s*<br>/g,
                              '$1</p><p style="color: #374151; font-size: 15px; line-height: 1.7; margin: 12px 0; text-align: justify; text-indent: 20px;">',
                            )
                            .replace(
                              /^/,
                              '<p style="color: #374151; font-size: 15px; line-height: 1.7; margin: 12px 0; text-align: justify; text-indent: 20px;">',
                            )
                            .replace(/$/, '</p>'),
                        }}
                      />
        ...
        >
          ...
          {isLoading ? (
            <div style={{ textAlign: 'center', padding: '40px' }}>
              ...
              ></div>
              <p style={{ color: 'white', marginTop: '15px' }}>Recherche en cours...</p>
            </div>
          ) : results.length > 0 ? (
            <div>
              <h2
            ...
            >
              <h3 style={{ color: '#6b7280' }}>🔍 Aucun résultat pour "{searchTerm}"</h3>
